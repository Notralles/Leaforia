...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # EfficientNet için standart normalizasyon
])
...
# EfficientNet modelini yükleme (Pretrained weights ile)
model = models.efficientnet_b0(pretrained=True)

# Modeli yeniden yapılandırma (son katmanı değiştirme)
num_ftrs = model.classifier[1].in_features
model.classifier[1] = nn.Linear(num_ftrs, len(full_train_dataset.classes))  # Kendi sınıf sayınızı buraya koyun
